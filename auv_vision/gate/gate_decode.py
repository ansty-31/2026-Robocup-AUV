...
class GateKeypointBackend(object):
    ...
    def __init__(self, path, labels, kpt_order, camera,
        ...
        #   ③ `vision.gate.keypoint.conf_thr`（= 历史行为：解码与模式判定同一个阈值）
        #   ④ 0.5
    ...
    @staticmethod
    def resolve_vis_thr(vis_thr=None):
        """解析解码期角点阈值（优先级见 __init__ 注释；非法值逐级回退，不崩）。"""
        if vis_thr is not None:
            try:
                return float(vis_thr)
        ...
        if v is not None:
            try:
                ...
                pass          # 配了个非法值 → 当作没配，回落到 conf_thr
        try:
            return float(S.get("vision.gate.keypoint.conf_thr", 0.5))
        except (TypeError, ValueError):
            return 0.5
