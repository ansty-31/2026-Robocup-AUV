...
stateDiagram-v2
    ...
    PASSING --> PASSING: 放大、缺点或无门框 / CONTINUE_PASS
...
| 阶段/观察 | 行为 |
|---|---|
| 找门：框高≥90%，上、下边缘均贴近图像边界 | `BACKWARD`，优先于转向和升降 |
| 找门：TL、BL 有，TR、BR 无 | `TURN_RIGHT` |
| 找门：TR、BR 有，TL、BL 无 | `TURN_LEFT` |
| 找门：仅 TL、TR 有，BR、BL 无 | `DESCEND`，下降 |
| 找门：仅 BR、BL 有，TL、TR 无 | `ASCEND`，上升 |
...
| 对准：丢点或 PnP 不可靠 | 回到找门流程，允许重新转向、升降、后退或搜索 |
| 对准持续达标 | 一次 `START_PASS`，进入 `PASSING` |
| 前进过门：有效图像中门框放大、裁切、只剩任一边或消失 | `CONTINUE_PASS`，不再转向、升降、后退或搜索 |
...
关键点数组固定按 **TL、TR、BR、BL** 排列。这里使用模型给出的角点身份，不根据当前画面中的像素高低或左右位置重新命名。下降仅对应 `[True, True, False, False]`，上升仅对应 `[False, False, True, True]`；另外两个点都必须不可用。若只有单点、对角点或三个点可用，不会触发升降。

“框高很大且上下贴边”只是基于检测框的近距离裁切判据，不是水下距离测量。只有缺点而没有上下贴边证据时，不触发后退。模型预测点的高置信度也不能代替实际训练质量验证。

例如，在找门阶段、检测框不满足后退条件且每次都是新的有效帧时：
...
| 0.1 | TL、TR | `HOLD` |
| 0.2 | TL、TR | `DESCEND` |
| 0.3 | BR、BL | `HOLD`，条件改变，重新累计 |
| 0.4 | BR、BL | `HOLD` |
| 0.5 | BR、BL | `ASCEND` |

输入故障同样清除连续确认计数。进入 `PASSING` 后，上述有效帧均输出 `CONTINUE_PASS`；输入故障输出 `HOLD` 并保留 `PASSING`。
...
外部控制程序的动作分支需接收新增的 `Action.DESCEND`（下降）和 `Action.ASCEND`（上升）；JSON 中的动作值分别为 `"DESCEND"`、`"ASCEND"`。两者只给出方向决策，不提供速度、目标深度或推进器输出；这些参数由外部控制程序决定。
...
自动测试使用合成投影和符合 Ultralytics 结构的测试结果对象，覆盖识别、对准、恢复、过门锁定、帧时序和输入故障。升降用例还覆盖时间及帧数门槛、上下条件交替、故障后重计数、全部16种可见性组合、置信度及坐标有效性、角点身份、后退优先级、对准失点恢复和过门后禁用升降。`verification.json` 和 `test-results.txt` 记录本次完整实跑结果。
