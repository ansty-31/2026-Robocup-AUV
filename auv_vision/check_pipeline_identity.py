# -*- coding: utf-8 -*-
"""gate/check_pipeline_identity.py — 移植后"域自证"脚本（不入运行时）

每次把代码/配置同步到另一台机器（PC ↔ 板端）后跑一次（服务整个项目，不限 gate）：
    python3 check_pipeline_identity.py
    # 或: python3 check_pipeline_identity.py --vision cfg/vision.yaml
...
_ROOT = os.path.dirname(os.path.abspath(__file__))   # 本脚本放在工程根
